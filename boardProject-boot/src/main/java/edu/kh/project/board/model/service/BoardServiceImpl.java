...
public class BoardServiceImpl implements BoardService{
	...
	public Board selectOne(Map<String, Integer> map) {
		...
		//2. 수행하려는 SQL이
		// 1) 모두 SELECT 이면서 
		// 2) 먼저 조회된 결과 중 일부를 이용해서 나중에 수행되는 SQL의 조건으로 삼을 수 ㅇ씨을 때 
		// --> Mybatis의 <resultMap>, <collection> 태그를 이용해서 Mapper 메서드 1회 호출로 여러 SELECT 한 번에 수행 가능
	...
	public Map<String, Object> searchList(Map<String, Object> paramMap, int cp) {
				...
				// 1. 지정된 게시판(boardCode)에서 검색조건에 맞으면서 삭제되지 않은 게시글 수를 조회
