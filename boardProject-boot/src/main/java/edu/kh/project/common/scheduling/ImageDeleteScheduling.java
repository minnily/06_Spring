...
public class ImageDeleteScheduling {
	...
	//@Scheduled(fixedDelay =  5000) // 이전 작업 끝나나 후 5초 후에 수행
	//@Scheduled(fixedRate = 5000 )// 이전 작업이 끝난 후 5초
	//cron="초 분 시 일 월 요일 [년도]" - 요일 : 1(SUN) ~ 7(SAT)
	//@Scheduled(cron ="0,15,30,45 * * * * *") 여기서  0,15,30,45은 전부 초를 뜻하는 곳이고 시계 초 단위가  0,15,30,45 경우 수행한다는 말이다.
	//@Scheduled(cron ="0 0 * * * *") // 0초 0분 => 즉. 정시마다 수행한다는 말이다.
	//@Scheduled(cron ="0 0 0 * * *") 자정 @Scheduled(cron ="0 0 12 * * *") 정오
	//@Scheduled(cron ="0 0 0 1 * *") 매달 1일
	@Scheduled(cron ="0 0 0 1 * *") // 초단위가 0,30초 인 경우(테스트용)
	public void scheduling() {
		...
		//imageArr라는 빈 배열을 boardArr와 member Arr 길이 만큼의 크기로 만든다.
		File[] imageArr = new File[boardArr.length + memberArr.length];
		
		// 배열 내용 복사(깊은 복사)
		// System.arraycopy(복사할 배열, 몇번 인덱스 부터 복사할지, 새로운 배열, 새로운 배열의 몇번부터 넣을지 인덱스 , 복사를 어디까지 할지)
